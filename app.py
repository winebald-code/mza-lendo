...
class Config:
    ...
    SEED_ADMIN_EMAIL = os.environ.get("SEED_ADMIN_EMAIL", "info@winebald.tech")
    SEED_ADMIN_USERNAME = os.environ.get("SEED_ADMIN_USERNAME", "winebald")
    SEED_ADMIN_PASSWORD = os.environ.get("SEED_ADMIN_PASSWORD", "223011005@Winebald")
...
TZ_LABEL = os.environ.get("TZ_LABEL", "EAT")
...
class AfricasTalking:
    ...
    to the SMS log and shown in the dashboard, which keeps demos honest and
...
def seed_starter_data(fac: Factory):
    """Minimal scaffolding so a brand new plant is not an empty room."""
    if Material.query.filter_by(factory_id=fac.id).count():
        return
    starters = [
        ("MS-001", "Mild steel sheet 2mm", "Metal", "sheet", 24, 10, 40, 1850),
        ("MS-002", "Square tube 25x25", "Metal", "m", 120, 60, 200, 320),
        ("HW-001", "Hinges", "Hardware", "pcs", 200, 80, 300, 95),
        ("PT-001", "Enamel paint, black", "Finishing", "litre", 18, 8, 30, 780),
        ("CN-001", "Welding rods 2.5mm", "Consumable", "kg", 12, 6, 25, 460),
    ]
    for code, name, cat, unit, qty, minimum, reorder, cost in starters:
        db.session.add(Material(factory_id=fac.id, code=code, name=name, category=cat,
                                unit=unit, quantity=qty, min_stock=minimum,
                                reorder_qty=reorder, unit_cost=cost, location="Store A"))
    db.session.add(Machine(factory_id=fac.id, code="MC-01", name="Bench grinder",
                           kind="Grinding", location="Bay 1", status="idle",
                           last_service_at=_today() - timedelta(days=20),
                           service_interval_days=90))
    db.session.add(Machine(factory_id=fac.id, code="MC-02", name="Arc welder",
                           kind="Welding", location="Bay 2", status="idle",
                           last_service_at=_today() - timedelta(days=45),
...
def cli_init_db():
    """Create tables and seed the super administrator (plus demo data)."""
    bootstrap()
